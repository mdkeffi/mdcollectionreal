...
const AgbadaPage = () => {
  ...
  useEffect(() => {
    ...
    const pendingOrderData = localStorage.getItem("agbadaOrder");
  ...
  const continueOrder = () => {
    if (pendingOrder) {
      ...
      navigate(`/payment?design=${encodeURIComponent(item.image)}&name=${encodeURIComponent(item.name)}&amount=${item.price}&type=agbada`);
    }
  };
